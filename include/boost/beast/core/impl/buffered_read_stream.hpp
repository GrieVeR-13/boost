        ...
        std::size_t bytes_transferred)
    {
        ...
        case 0:
            if(s_.buffer.size() == 0)
            {
                ...
                step_ = 2;
                return s_.next_layer_.async_read_some(
                    s_.buffer.prepare(read_size(
                            s_.buffer, s_.capacity_)),
                            std::move(*this));
            }
            step_ = 3;
            return net::post(
        ...
        case 2:
            s_.buffer.commit(bytes_transferred);
            BOOST_FALLTHROUGH;

        case 3:
            bytes_transferred =
                net::buffer_copy(b_, s_.buffer.data());
            s_.buffer.consume(bytes_transferred);
            break;
        }
